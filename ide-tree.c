                      ...
                      GtkListView *list_view)
{
  ...
  model = G_LIST_MODEL (priv->addins);
  ...
  for (guint i = 0; i < n_items; i++)
    {
      ...
      if (ide_tree_addin_node_activated (addin, self, node))
        break;
    }
}
...
ide_tree_root (GtkWidget *widget)
{
  ...
  if (priv->root != NULL && !priv->did_root_expand)
    {
      priv->did_root_expand = TRUE;
                            ...
                            gpointer         user_data)
{
  ...
  if (gtk_gesture_get_sequence_state (GTK_GESTURE (click), sequence) == GTK_EVENT_SEQUENCE_NONE)
    {
      GListModel *model = G_LIST_MODEL (priv->addins);
  ...
  for (guint i = 0; i < n_items; i++)
    {
      ...
      if ((provider = ide_tree_addin_node_draggable (addin, node)))
...
ide_tree_init (IdeTree *self)
{
  ...
  priv->addins = g_list_store_new (IDE_TYPE_TREE_ADDIN);
...
void
ide_tree_add_addin (IdeTree      *self,
                    IdeTreeAddin *addin)
{
  IdeTreePrivate *priv = ide_tree_get_instance_private (self);

  g_return_if_fail (IDE_IS_TREE (self));
  g_return_if_fail (IDE_IS_TREE_ADDIN (addin));

  g_list_store_append (priv->addins, addin);

  ide_tree_addin_load (addin, self);
}

void
ide_tree_remove_addin (IdeTree      *self,
                       IdeTreeAddin *addin)
{
  IdeTreePrivate *priv = ide_tree_get_instance_private (self);
  GListModel *model;
  ...
  for (guint i = 0; i < n_items; i++)
    {
      g_autoptr(IdeTreeAddin) element = g_list_model_get_item (model, i);

      if (element == addin)
        {
          g_list_store_remove (priv->addins, i);
          break;
        }
    }
}
